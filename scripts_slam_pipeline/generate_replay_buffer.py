...
def _get_demo_name(plan_episode, default_name: str) -> str:
    """
    Try to extract a human-readable demo/episode name from the plan.
    Falls back to default_name if no known key exists.
    """
    for k in ["demo_name", "episode_name", "name", "demo_id", "episode_id", "id"]:
        if k in plan_episode and plan_episode[k] is not None:
            return str(plan_episode[k])
    return default_name
...
def main(input, output, out_res, out_fov, compression_level,
    ...
    # === MODIFICATION START ===
    # Fix: intrinsics path should not depend on the per-episode ipath variable.
    # Use ROOT_DIR (or change this to wherever your calibration folder lives).
    ...
    for ipath in input:
        ...
        for ep_idx, plan_episode in enumerate(plan):
            print(ipath.name, ep_idx, plan_episode.get("episode_timestamps", None).shape)
            ...
            for cam_id, camera in enumerate(cameras):
                ...
                # Ignore original video_end
                video_start, _ = camera["video_start_end"]

                # Recalculate end based on sliced data length
            ...
            for frame_idx, frame in tqdm(
                ...
                if frame_idx < tasks[curr_task_idx]["frame_start"]:
                    ...
                    img = draw_predefined_mask(
                        img, color=(0, 0, 0), mirror=no_mirror, gripper=True, finger=False
                    )
                    ...
                    raise AssertionError("Frame index exceeded expected task bounds")
